...
M = cv2.getPerspectiveTransform(src, dst) # Calculates a perspective transform from four pairs of the corresponding points
...
def abs_sobel_thresh(img, orient='x', sobel_kernel=3, thresh=(0, 255)):
    ...
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY) # Converts an image from one color space to another
...
def mag_thresh(img, sobel_kernel=3, thresh=(0, 255)):
    ...
    sobelx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=sobel_kernel) # Calculates the first, second, third, or mixed image derivatives using an extended Sobel operator
...
def gradient_pipeline(image, ksize = 3, sx_thresh=(20, 100), sy_thresh=(20, 100), m_thresh=(30, 100), dir_thresh=(0.7, 1.3)):
    ...
    combined = np.zeros_like(mag_binary) # Return an array of zeros with the same shape and type as a given array.
...
def full_search(binary_warped, visualization=False):
    ...
    out_img = np.dstack((binary_warped, binary_warped, binary_warped))*255 # Stack arrays in sequence depth wise (along third axis)
    out_img = out_img.astype('uint8')

    # Find the peak of the left and right halves of the histogram
    # These will be the starting point for the left and right lines
    midpoint = np.int(histogram.shape[0]/2)
    leftx_base = np.argmax(histogram[:midpoint]) # Returns the indices of the maximum values along an axis
    ...
    left_fit = np.polyfit(lefty, leftx, 2) # Least squares polynomial fit.
...
def measure_lane_curvature(ploty, leftx, rightx, visualization=False):
    ...
    if leftx[0] - leftx[-1] > 50/input_scale:
        curve_direction = 'Turning Left'
    elif leftx[-1] - leftx[0] > 50/input_scale:
        curve_direction = 'Turning Right'
    else:
        curve_direction = 'Going Straight'
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    # draw background
    color_warp = np.zeros_like(undist_ori).astype(np.uint8) # transfer data type from string to numeric

    # create a frame to hold every image
    whole_frame = np.zeros((int(h*2.5), int(w*2.34), 3), dtype=np.uint8)
    # The dimension of the pts is the area of the green or red lane
